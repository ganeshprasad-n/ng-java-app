#!/bin/bash
################################################################################
# VProfile Java Application - App Server Setup Script
# 
# Purpose: Install and configure all services for VProfile application
# OS: Fedora 42
...
echo "=============================================="
echo "VProfile App Server Setup - Fedora 42"
...
################################################################################
echo "[Step 4/10] Installing Adoptium Temurin Java 11 and Maven..."
...
if sudo systemctl is-active --quiet elasticsearch; then
    ...
    sudo rpm --import https://artifacts.elastic.co/GPG-KEY-elasticsearch 2>/dev/null || true
...
################################################################################
# Step 10: Create deploy user for Jenkins
################################################################################
...
echo ""
echo "NEXT STEPS:"
echo "  1. Run verification script: ./verify-app-server.sh"
echo "  2. Configure application with IP: $CURRENT_IP"
echo "  3. Update Jenkins SSH config with this IP"
echo "  4. Update GitHub config repo with this IP"
echo ""
echo "=============================================="
